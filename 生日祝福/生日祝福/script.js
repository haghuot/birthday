  ...
  function init() {
    ...
    const base =
      location.origin && location.origin !== "null"
        ...
        : "https://你的域名或平台/index.html";
    const example = `${base}?to=${encodeURIComponent("小明")}&from=${encodeURIComponent("老朋友")}&title=${encodeURIComponent(
      "生日快乐"
    )}&msg=${encodeURIComponent("愿你眼里有光，心里有爱。\\n新的一岁，继续闪闪发光！")}`;
    if (els.exampleUrl) els.exampleUrl.textContent = example;
    ...
    if (els.btnCopyLink) {
      els.btnCopyLink.addEventListener("click", async () => {
        ...
        toast(ok ? "链接已复制，直接发给对方即可" : "复制失败：请手动复制地址栏链接");
      });
    }
